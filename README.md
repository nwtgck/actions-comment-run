...
By default, only the owner can execute the scripts. You can change `allowed-associations: '["OWNER"]'` in the yaml above, or add users who can run scripts regardless of their comment association via `allowed-users` (this is useful in org-owned repos, where only the organization itself has `OWNER` permission).
